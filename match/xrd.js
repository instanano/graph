(function (G) {
    ...
    const setProgress = (p) => {
        const btn = document.getElementById('xrd-search-btn');
        if (!btn) return;
        if (p === 'done') { btn.classList.add('progress-done'); setTimeout(() => { btn.classList.remove('progress-done'); btn.style.setProperty('--progress', '0%'); }, 500); }
        else { btn.classList.remove('progress-done'); btn.style.setProperty('--progress', p + '%'); }
    };
